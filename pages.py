...
body{font-family:system-ui,-apple-system,sans-serif;background:#0a0a1a;color:#e0f0ff;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}
.card{background:rgba(15,25,50,0.8);border:1px solid rgba(0,240,255,0.15);border-radius:16px;padding:30px;max-width:500px;width:100%;backdrop-filter:blur(20px)}
h1{font-size:22px;margin-bottom:5px;background:linear-gradient(135deg,#00f0ff,#ff00ff);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.sub{color:#667;font-size:13px;margin-bottom:20px}
.locked{background:rgba(255,0,100,0.1);border:1px solid rgba(255,0,100,0.2);border-radius:10px;padding:20px;text-align:center}
.locked input{background:rgba(0,0,0,0.3);border:1px solid rgba(0,240,255,0.2);border-radius:8px;padding:10px 14px;color:#fff;width:100%;margin:10px 0;font-size:14px}
.locked button{background:linear-gradient(135deg,#00f0ff,#8b5cf6);color:#fff;border:none;border-radius:8px;padding:10px 24px;cursor:pointer;font-size:14px}
.link-item{background:rgba(0,0,0,0.2);border:1px solid rgba(0,240,255,0.1);border-radius:10px;padding:14px;margin-bottom:10px}
...
.link-info{font-size:12px;color:#889;color:#8899aa;line-height:1.8}
.link-url{font-size:11px;color:#00f0ff;background:rgba(0,240,255,0.08);padding:6px 10px;border-radius:6px;margin-top:8px;word-break:break-all;font-family:monospace;cursor:pointer}
.link-url:hover{background:rgba(0,240,255,0.15)}
.btn{display:inline-block;background:linear-gradient(135deg,#00f0ff,#8b5cf6);color:#fff;border:none;border-radius:8px;padding:8px 16px;cursor:pointer;font-size:12px;margin-top:8px;text-decoration:none}
</style>
</head>
<body>
<div class="card" id="app">
<div id="loading" style="text-align:center;padding:40px"><div style="font-size:24px">Loading...</div></div>
</div>
<script>
const UK='""" + uuid_key + r"""';
async function init(){
  try{
    ...
    if(d.locked){
      document.getElementById('app').innerHTML=`<div class="locked"><h1 style="margin-bottom:10px">🔒 ${d.name}</h1><p style="color:#889;font-size:13px">This group is password protected</p><input type="password" id="pw" placeholder="Enter password"><button onclick="unlock()">Unlock</button></div>`;
      return;
    }
    render(d);
  }catch(e){document.getElementById('app').innerHTML='<div style="text-align:center;padding:40px;color:#ff0066">Error loading</div>';}
}
...
function render(d){
  ...
  d.links.forEach(l=>{
    ...
    html+=`<div class="link-item"><div class="link-name">${status} ${l.label}</div><div class="link-info">${l.used_fmt} / ${l.limit_fmt} &bull; ${l.protocol} &bull; ${l.connections} connections</div><div class="link-url" onclick="copy(this)">${l.vless_link}</div><a class="btn" href="${l.sub_url}" target="_blank">Sub Link</a></div>`;
  });
  document.getElementById('app').innerHTML=html;
}
function copy(el){navigator.clipboard.writeText(el.textContent).then(()=>{el.style.background='rgba(0,240,255,0.25)';setTimeout(()=>{el.style.background=''},800)});}
init();
</script>
</body></html>"""
...
body{font-family:'Inter',system-ui,sans-serif;background:var(--bg);min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;overflow:hidden}
...
.brand-icon{width:52px;height:52px;border-radius:14px;background:linear-gradient(135deg,#00f0ff,#8b5cf6);display:flex;align-items:center;justify-content:center;font-size:24px;font-weight:800;color:#fff;box-shadow:0 0 30px rgba(0,240,255,0.3),0 0 20px rgba(139,92,246,0.2)}
...
.hint-val{font-family:ui-monospace,monospace;font-size:14px;font-weight:700;color:var(--accent);background:rgba(0,240,255,0.08);border:1px solid rgba(0,240,255,0.2);padding:4px 14px;border-radius:8px;cursor:pointer;transition:.15s;letter-spacing:.08em}
...
<div class="wrap">
  <div class="card">
    <div class="brand">
      ...
      <div><div class="brand-name">Cyber-Rage</div><div class="brand-sub">v1.0 GATEWAY</div></div>
    </div>
    <h1>Access Panel</h1>
    ...
    <form id="form">
      ...
      <button class="btn" type="submit" id="btn">Login to Dashboard</button>
    </form>
...
document.getElementById('form').addEventListener('submit',async e=>{
  ...
  btn.innerHTML='<span style="animation:spin 1s linear infinite;display:inline-block">&#9696;</span> Authenticating...';
  try{
    ...
    btn.disabled=false;btn.innerHTML='Login to Dashboard';
  }
});
</script>
</body></html>"""
...
body{font-family:'Inter',system-ui,sans-serif;background:var(--bg);color:var(--t1);min-height:100vh;display:flex;font-size:14px}
...
.nav-it i{font-size:16px;width:20px;text-align:center;flex-shrink:0}
...
.sb-btn{display:flex;align-items:center;justify-content:center;gap:8px;border-radius:10px;padding:10px;font-size:12.5px;font-weight:500;font-family:inherit;border:1px solid var(--card-b);cursor:pointer;width:100%;transition:.15s;margin-bottom:6px}
...
.mob-top{display:none;position:fixed;top:0;right:0;left:0;height:54px;background:var(--bg2);border-bottom:1px solid var(--card-b);z-index:150;align-items:center;justify-content:space-between;padding:0 16px}
...
.menu-btn,.theme-mob{background:var(--accent-d);border:1px solid var(--card-b);color:var(--t2);width:36px;height:36px;border-radius:9px;font-size:18px;display:flex;align-items:center;justify-content:center;cursor:pointer}
.overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:190;backdrop-filter:blur(4px)}
.overlay.show{display:block}
.main{margin-right:var(--sidebar-w);flex:1;padding:30px 30px 70px;min-width:0}
.pg{display:none}
...
.tb-title{font-size:20px;font-weight:700;color:var(--t1);letter-spacing:-.02em}
...
.vl-code{background:rgba(0,0,0,.2);border:1px solid var(--card-b);border-radius:10px;padding:14px 16px;font-size:11.5px;font-family:ui-monospace,monospace,color:var(--accent);word-break:break-all;line-height:1.8;letter-spacing:.01em}
...
.card{background:var(--card);border:1px solid var(--card-b);border-radius:var(--radius);padding:20px;transition:border-color .2s}
...
.tog{width:20px;height:36px;border-radius:20px;background:rgba(100,116,139,0.25);position:relative;cursor:pointer;transition:.2s;flex-shrink:0;border:none}
.tog::after{content:'';position:absolute;width:14px;height:14px;border-radius:50%;background:#fff;left:3px;bottom:3px;transition:.2s;box-shadow:0 1px 4px rgba(0,0,0,.3)}
...
.list-item{display:flex;align-items:center;gap:12px;padding:12px 14px;border:1px solid var(--card-b);border-radius:12px;margin-bottom:8px;transition:.15s;cursor:default}
...
.list-name{font-size:13px;font-weight:600;color:var(--t1);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.list-sub{font-size:11px;color:var(--t3);margin-top:2px}
...
.modal{background:var(--bg2);border:1px solid var(--card-b);border-radius:20px;padding:28px;width:90%;max-width:520px;max-height:85vh;overflow-y:auto}
...
.conn-ip{font-family:ui-monospace,monospace;color:var(--accent);font-weight:600;min-width:120px}
.conn-label{color:var(--t2);flex:1}
.conn-bytes{color:var(--t1);font-weight:600;font-family:ui-monospace,monospace;font-size:11px}
...
.log-time{color:var(--t3);font-family:ui-monospace,monospace;font-size:10px;white-space:nowrap;flex-shrink:0}
...
<div class="mob-top">
  ...
  <div class="mob-right">
    <button class="theme-mob" onclick="toggleTheme()">&#9790;</button>
...
<aside class="sidebar" id="sidebar">
  <div class="logo">
    ...
    <div><div class="logo-name">Cyber-Rage</div><div class="logo-sub">v1.0 GATEWAY</div></div>
    <button class="sb-close" onclick="toggleSidebar()">&#10005;</button>
  </div>
  <div class="nav-wrap">
    <div class="nav-sec">Main</div>
    <div class="nav-it on" data-pg="overview" onclick="showPg(this)">&#9673; Overview</div>
    <div class="nav-it" data-pg="connections" onclick="showPg(this)">&#9881; Connections <span class="nav-badge" id="conn-badge">0</span></div>
    <div class="nav-sec">Management</div>
    <div class="nav-it" data-pg="links" onclick="showPg(this)">&#128279; Configs <span class="nav-badge" id="links-badge">0</span></div>
    <div class="nav-it" data-pg="subs" onclick="showPg(this)">&#128194; Groups <span class="nav-badge" id="subs-badge">0</span></div>
    <div class="nav-sec">Monitoring</div>
    <div class="nav-it" data-pg="traffic" onclick="showPg(this)">&#128200; Traffic</div>
    <div class="nav-it" data-pg="activity" onclick="showPg(this)">&#128221; Activity</div>
    <div class="nav-it" data-pg="errors" onclick="showPg(this)">&#9888; Errors</div>
  </div>
  <div class="sb-foot">
    <button class="sb-btn" onclick="toggleTheme()">&#9790; Toggle Theme</button>
    <button class="sb-btn sb-logout" onclick="logout()">&#9211; Logout</button>
  </div>
</aside>
...
<div class="pg on" id="pg-overview">
  <div class="topbar"><div><div class="tb-title">&#9673; Dashboard</div><div class="tb-sub">Cyber-Rage Gateway &bull; <span id="uptime">--:--:--</span></div></div></div>
  <div class="metrics">
    <div class="metric"><div class="m-icon">&#9881;</div><div class="m-label">Active Connections</div><div class="m-val" id="m-conn">0</div></div>
    <div class="metric suc"><div class="m-icon suc">&#128200;</div><div class="m-label">Total Traffic</div><div class="m-val" id="m-traffic">0 <span class="m-unit">MB</span></div></div>
    <div class="metric"><div class="m-icon pur">&#128279;</div><div class="m-label">Configs</div><div class="m-val" id="m-links">0</div><div class="m-sub"><span id="m-active" style="color:var(--green)">0</span> active &bull; <span id="m-expired" style="color:var(--red)">0</span> expired</div></div>
    <div class="metric dan"><div class="m-icon dan">&#9888;</div><div class="m-label">Errors</div><div class="m-val" id="m-errors">0</div><div class="m-sub"><span id="m-reqs">0</span> requests</div></div>
  </div>
  <div class="vless-box" id="default-link-box">
    <div class="vl-header"><div class="vl-title">&#128279; Default VLESS Link</div><span class="badge bg-green"><span class="dot dg pulse"></span> Active</span></div>
    <div class="vl-code" id="default-link-code">Loading...</div>
    <div class="vl-actions">
      <button class="btn btn-p" onclick="copyDefaultLink()">&#128203; Copy Link</button>
      <button class="btn btn-o" onclick="copySubUrl()">&#128279; Copy Sub URL</button>
    </div>
  </div>
  <div class="g3">
    <div class="card"><div class="card-title"><i>&#128200;</i> Traffic (Last 24h)</div><div class="ch-lg"><canvas id="trafficChart"></canvas></div></div>
    <div class="card"><div class="card-title"><i>&#128221;</i> Recent Activity</div><div id="recent-activity" style="max-height:340px;overflow-y:auto"></div></div>
  </div>
</div>

<!-- CONNECTIONS -->
<div class="pg" id="pg-connections">
  <div class="topbar"><div><div class="tb-title">&#9881; Live Connections</div><div class="tb-sub">Real-time active connections grouped by IP</div></div></div>
  <div class="card" id="conn-list"><div class="empty"><i>&#9881;</i>No active connections</div></div>
</div>

<!-- LINKS -->
<div class="pg" id="pg-links">
  <div class="topbar">
    <div><div class="tb-title">&#128279; Config Management</div><div class="tb-sub">Create, edit, and manage VLESS configs</div></div>
    <button class="btn btn-p" onclick="showCreateLink()">+ New Config</button>
  </div>
...
<div class="pg" id="pg-subs">
  <div class="topbar">
    <div><div class="tb-title">&#128194; Subscription Groups</div><div class="tb-sub">Group configs and generate public pages</div></div>
    <button class="btn btn-p" onclick="showCreateSub()">+ New Group</button>
  </div>
...
<div class="pg" id="pg-traffic">
  <div class="topbar"><div><div class="tb-title">&#128200; Traffic Analysis</div><div class="tb-sub">Hourly traffic breakdown</div></div></div>
  <div class="card"><div class="ch-lg"><canvas id="trafficChartFull"></canvas></div></div>
</div>

<!-- ACTIVITY -->
<div class="pg" id="pg-activity">
  <div class="topbar"><div><div class="tb-title">&#128221; Activity Log</div><div class="tb-sub">Recent system events and actions</div></div></div>
  <div class="card" id="activity-list"><div class="empty"><i>&#128221;</i>No activity yet</div></div>
</div>

<!-- ERRORS -->
<div class="pg" id="pg-errors">
  <div class="topbar"><div><div class="tb-title">&#9888; Error Log</div><div class="tb-sub">Recent errors and warnings</div></div></div>
  <div class="card" id="errors-list"><div class="empty"><i>&#9888;</i>No errors</div></div>
</div>
</main>
...
<div class="modal-bg" id="modal-create-link">
  <div class="modal">
    ...
    <div class="form-row" style="margin-top:8px">
      <button class="btn btn-p" onclick="createLink()">Create Config</button>
      <button class="btn btn-o" onclick="closeModal('modal-create-link')">Cancel</button>
    </div>
  </div>
</div>
...
<div class="modal-bg" id="modal-edit-link">
  <div class="modal">
    ...
    <div class="form-row" style="margin-top:8px">
      <button class="btn btn-p" onclick="updateLink()">Save Changes</button>
...
<div class="modal-bg" id="modal-create-sub">
  <div class="modal">
    <h2><i>&#128194;</i> New Subscription Group</h2>
    <div class="form-row">
    ...
    <div class="form-row" style="margin-top:8px">
      <button class="btn btn-p" onclick="createSub()">Create Group</button>
      <button class="btn btn-o" onclick="closeModal('modal-create-sub')">Cancel</button>
    </div>
  </div>
</div>
...
async function loadConnections(){
  try{
    ...
    const el=$('#conn-list');
    if(!d.connections.length){el.innerHTML='<div class="empty"><i>&#9881;</i>No active connections</div>';return;}
...
async function loadActivity(){
  try{
    ...
    if(!d.logs.length){el.innerHTML='<div class="empty"><i>&#128221;</i>No activity yet</div>';return;}
...
function renderLinks(){
  const el=$('#links-list');
  if(!linksData.length){el.innerHTML='<div class="empty"><i>&#128279;</i>No configs yet. Create one!</div>';return;}
  el.innerHTML=linksData.map(l=>{
    ...
    return `<div class="list-item">
      ...
      <div class="list-info">
        ...
        <div class="list-sub">${l.protocol} &bull; ${l.fingerprint} &bull; Port ${l.port} &bull; ${l.used_fmt||'0 B'} / ${limit} &bull; ${l.connected_ips||0} IPs</div>
      </div>
      <div class="list-actions">
        <button class="btn btn-sm btn-g" onclick="copyText('${l.vless_link.replace(/'/g,"\\'")}')" title="Copy Link">&#128203;</button>
        <button class="btn btn-sm btn-o" onclick="editLink('${l.uuid}')" title="Edit">&#9998;</button>
        <button class="btn btn-sm ${l.active?'btn-o':'btn-g'}" onclick="toggleLink('${l.uuid}',${!l.active})" title="${l.active?'Disable':'Enable'}">${l.active?'&#9211;':'&#9654;'}</button>
        <button class="btn btn-sm btn-d" onclick="deleteLink('${l.uuid}')" title="Delete">&#128465;</button>
      </div>
    </div>`;
  }).join('');
}
...
function renderSubs(){
  const el=$('#subs-list');
  if(!subsData.length){el.innerHTML='<div class="empty"><i>&#128194;</i>No groups yet. Create one!</div>';return;}
  el.innerHTML=subsData.map(s=>{
    return `<div class="list-item">
      <div class="list-icon">&#128194;</div>
      <div class="list-info">
        <div class="list-name">${s.name} ${s.has_password?'&#128274;':''}</div>
      ...
      <div class="list-actions">
        <button class="btn btn-sm btn-g" onclick="copyText('${s.sub_url.replace(/'/g,"\\'")}')" title="Copy Sub URL">&#128203;</button>
        <button class="btn btn-sm btn-d" onclick="deleteSub('${s.sub_id}')" title="Delete">&#128465;</button>
      </div>
    </div>`;
  }).join('');
}
...
function updateTrafficChart(hourly){
  ...
  trafficChart=new Chart(ctx,{type:'line',data:{labels,datasets:[{label:'Traffic (MB)',data:data.map(v=>v/1024/1024),borderColor:'#00f0ff',backgroundColor:'rgba(0,240,255,0.08)',fill:true,tension:.4,pointRadius:2,pointBackgroundColor:'#00f0ff',borderWidth:2}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{grid:{color:'rgba(0,240,255,0.04)'},ticks:{color:'#556688',font:{size:10}}},y:{grid:{color:'rgba(0,240,255,0.04)'},ticks:{color:'#556688',font:{size:10},callback:v=>v.toFixed(1)}}}}});
}
...
async function loadTrafficFull(){
  try{
    ...
    trafficChartFull=new Chart(ctx,{type:'bar',data:{labels,datasets:[{label:'Traffic (MB)',data,backgroundColor:'rgba(0,240,255,0.3)',borderColor:'#00f0ff',borderWidth:1,borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{grid:{color:'rgba(0,240,255,0.04)'},ticks:{color:'#556688',font:{size:10}}},y:{grid:{color:'rgba(0,240,255,0.04)'},ticks:{color:'#556688',font:{size:10},callback:v=>v.toFixed(1)}}}}});
  }catch(e){}
}
